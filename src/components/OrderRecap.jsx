...
export default function OrderRecap() {
  ...
  return (
    <>
      <div className="flex flex-col justify-between flex-grow text-gray-500 bg-[#2d2d2d] p-4 rounded-md transition-all duration-500 delay-200 ease-in-out overflow-hidden">
        <div
          className={`flex flex-col ${
            cart.length
              ? "opacity-100 translate-y-0 "
              : "opacity-0 -translate-y-10"
          } delay-200  space-y-4 duration-1000 transition-all text-sm `}
        >
        ...
        <div
          className={`flex flex-col ${
            cart.length ? "opacity-0" : "opacity-100"
          } duration-[2s] delay-500  text-xs items-center justify-center text-stone-400 transition hover:opacity-80`}
        >
          <svg
