...
export default function ProductItem({ productProps }) {
  ...
  function handleIncreaseQtyProduct(productParam) {
    ...
    let newCart = [...cart];
    const producIndex = newCart.findIndex(
  ...
  return (
    <>
      <div
        ...
        className="flex flex-col space-y-2 col-span-1 text-gray-500 bg-[#2d2d2d] p-2 rounded-lg relative"
      >
        <div
          className="h-40 w-full bg-cover bg-center rounded-lg"
          style={{ backgroundImage: `url(${productProps.urlImage})` }}
        ...
        <div className="flex justify-between items-center">
          <span className="font-bold">
            {`${productProps.devise} ${productProps.price}`}{" "}
          </span>
          <span className="text-[10px]">{productProps.available} item(s)</span>
        </div>
        <div
          className={`flex flex-col space-y-8 bg-[#f05756]/90 absolute left-0 ${
            showOverlay ? "h-4/5 opacity-100" : " h-0 opacity-0 z-[-2]"
          } delay-500 duration-300 transition-all ease-in-out bottom-0 w-full rounded-lg p-2`}
        >
          <span className="text-xl">{productProps.name} </span>
          <span className="font-bold text-2xl">{`${productProps.devise} ${
            productProps.price * qty
          }`}</span>
          <div className="flex justify-center space-x-8 items-center">
            <div
              onClick={() => handleDecreaseQtyProduct(productProps)}
              className="flex items-center justify-center h-10 w-10 rounded-xl border border-white bg-white text-stone-700 hover:opacity-60 transition"
            >
              -
            </div>
            <span className="text-4xl">{qty}</span>
            <div
              onClick={() => handleIncreaseQtyProduct(productProps)}
              className="flex items-center justify-center h-10 w-10 rounded-xl border border-white bg-white text-stone-700 hover:opacity-60 transition"
            >
              +
            </div>
